...
const OrganizerView = () => {
    ...
    const handleSaveEvent = async () => {
        setError('');
        try {
            await axios.put(`http://13.60.43.21:3001/api/events/${eventId}`, editForm);
    ...
    const handleAddParticipant = async () => {
        ...
        setError('');
        try {
            await axios.post(`http://13.60.43.21:3001/api/events/${eventId}/participants`, {
    ...
    const handleRemoveParticipant = async (userId) => {
        setError('');
        try {
            await axios.delete(`http://13.60.43.21:3001/api/events/${eventId}/participants/${userId}`);
            const res = await axios.get(`http://13.60.43.21:3001/api/events/${eventId}`);
            setParticipants(res.data.participants);
        } catch (err) {
